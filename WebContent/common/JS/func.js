...
function chkShowAll() {
    // すべてのチェックボックス要素を取得
    const employeeCodes = Array.from(document.getElementsByName("employeeCode"));
    
    // チェックされたチェックボックスがあるか確認
    const isChecked = employeeCodes.some(checkbox => checkbox.checked);

    // デバッグ用のログを出力
    console.log("チェックされているか？:", isChecked);

    if (!isChecked) {
        // 1つもチェックされていない場合、警告を表示し送信を中止
        alert("1つ以上の選択が必要です");
        return false;
    }
...
function setValue(value) {
    document.getElementsByName("chkBtn")[0].value = value;  // 隠しフィールドに値をセット
}
